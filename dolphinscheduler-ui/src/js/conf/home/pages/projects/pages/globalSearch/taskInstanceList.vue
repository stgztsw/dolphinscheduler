...
<script>
  ...
  export default {
    name: 'task-instance-global-list-index',
    data () {
      return {
        isLoading: true,
        total: null,
    ...
    props: {},
    methods: {
      ...mapActions('dag', ['getGlobalTaskInstance']),
      /**
       * click query
       */
      _onQuery (o) {
      ...
      _debounceGET: _.debounce(function (flag) {
        ...
        this._getList(flag)
        console.log(this.searchParams.searchVal)
        console.log(this.taskInstanceList.length)
      }, 100, {
        'leading': false,
        'trailing': true
      })
    ...
    created () {
      try {
        if (!_.isEmpty(this.$route.params)) {
          this.searchParams = _.assign(this.searchParams, this.$route.params.searchParams)
          // this.searchParams.searchVal = this.$route.params.searchParams
          this.taskInstanceList = _.assign(this.taskInstanceList, this.$route.params.taskInstanceList)
        }
      }catch (e) {
