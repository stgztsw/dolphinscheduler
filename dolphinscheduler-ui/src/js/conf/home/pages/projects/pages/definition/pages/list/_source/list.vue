      ...
      <table class="fixed">
        ...
        <tr v-for="(item, $index) in list" :key="item.id">
          ...
          <td style="z-index: inherit;">
            ...
            <x-button type="info" shape="circle" size="xsmall" data-toggle="tooltip" :title="$t('Timing')" @click="_timing(item)" :disabled="item.processType !== 'SCHEDULER' || item.releaseState !== 'ONLINE' || item.scheduleReleaseState !== null"  icon="ans-icon-timer"><!--{{$t('定时')}}--></x-button>
            ...
            <x-button type="info" shape="circle" size="xsmall" data-toggle="tooltip" :title="$t('Cron Manage')" @click="_timingManage(item)" :disabled="item.processType !== 'SCHEDULER' || item.releaseState !== 'ONLINE'"  icon="ans-icon-datetime"><!--{{$t('定时管理')}}--></x-button>
            <x-poptip
