...
# reader 支持ftp, oracle, hive,mysql,pg,sqlserver,cache
reader_db_type = "cache"
input_mode = "create_table_sql"
mysql_file_path = "C:\\Users\\assist.bi2\\Desktop\\SQL\\sql"  # table schema 文件夹路径

reader_db_table_name = "dbo.Bed"  # 注意oracle的识别大小写，请确保库表名的大小写和实际一致
# writer 支持hive, dorisdb,sftp
writer_db_type = "hive"
writer_hive_table_prefix = "ods_f_aws_"
writer_db_name = "jh_ods"
if reader_db_type == "ftp":
    writer_table_name = None
else:
    writer_table_name = writer_hive_table_prefix + reader_db_table_name.split(".")[
        1]  # 默认策略: hive的场景会在表名前加ods_; dorisdb的场景会在表前加 doris_
...
print("如果writer_table_name为None, 会按默认策略生成: hive的场景会在表名前加ods_; dorisdb的场景采用原表名")
...
db_infos = {
    ...
    "mysql": {
        "prd": {
            ...
            "useUnicode": "true",
            "useSSL": "false",
            "listener": "10.2.242.246:3306/datasyncinterface?useSSL=false&useUnicode=true&characterEncoding=UTF-8"
        }
    },
    "sqlserver": {
        "prd": {
            "host": "10.2.12.162",
            "user": "read_1216202",
            "password": "saqwJ@ccc_app",
            "dbname": "Interconnect",
            "listener": "10.2.12.162:1433;DatabaseName=Interconnect"
        }
    },
    "pg": {
        "prd": {
            "host": "10.2.2.94",
            "user": "jiahui_bi",
            "password": "jiahui_bi@JIAHUI6344",
            "port": "5432",
            "dbname": "PROD",
            "listener": "10.2.2.94:5432/PROD"
        }
    },
    "cache":{
        "prd": {
...
sftp_info = db_infos.get(writer_db_type).get(env)
...
def create_pg_reader_json():
    global column_type_list
    conn = pg.connect(database=reader_db_info.get('dbname')
                      , user=reader_db_info.get('user')
                      , password=reader_db_info.get('password')
    ...
    table_name = get_reader_db_table_name()[1].upper()
    # 获取表的元信息
    sql = "SELECT a.attname AS field, t.typname AS type, b.description AS comment \
            ...
            AND a.atttypid = t.oid" % table_name.lower()
    print("pg schema sql = %s" % sql)
    cursor.execute(sql)
    ...
    column_list = list(map(lambda x: x[1], column_type_list))
    connection = [
        {"table": [reader_db_table_name], "jdbcUrl": ["jdbc:postgresql://" + reader_db_info.get("listener")]}]
    pg_reader = {"name": "postgresqlreader", "parameter": {"username": reader_db_info.get("user"),
                                                           "password": reader_db_info.get("password"),
                                                           "column": column_list, "splitPk": column_list[0],
                                                           "connection": connection}}
    return pg_reader
...
def reader_type_mapping(data_type: str):
    ...
    index = data_type.find("(") #BIGINT(64)
    rtv_data_type = None
    data_len = None
    if index > 0:
    ...
    if reader_db_type.lower() == "hive":
        ...
        if data_type in ("INT", "TINYINT", "SMALLINT", "MEDIUMINT", "BIGINT"):
            ...
            rtv_data_type = "Boolean"
    elif reader_db_type.lower() == "mysql" or reader_db_type.lower() == "cache":
        if data_type in ("INT", "TINYINT", "SMALLINT", "MEDIUMINT", "BIGINT"):
            rtv_data_type = "Long"
        elif data_type in ("FLOAT", "DOUBLE", "DECIMAL"):
            rtv_data_type = "Double"
            ...
            rtv_data_type = "Bytes"
    elif reader_db_type.lower() == "pg":
        if data_type in ("BIGINT", "BIGSERIAL", "INT", "SMALLINT", "SERIAL", "INT4"):
            rtv_data_type = "Long"
        elif data_type in ("DOUBLE", "PRECISION", "MONEY", "NUMERIC", "REAL", "FLOAT"):
            rtv_data_type = "Double"
        elif data_type in ("VARCHAR", "CHAR", "TEXT", "BIT", "INET"):
            rtv_data_type = "String"
            ...
            rtv_data_type = "Bytes"
    elif reader_db_type.lower() == "sqlserver":
        if data_type in ("BIGINT", "INT", "SMALLINT", "TINYINT"):
            rtv_data_type = "Long"
        elif data_type in ("FLOAT", "DECIMAL", "REAL", "NUMERIC"):
            rtv_data_type = "Double"
        elif data_type in ("CHAR", "NCHAR", "NTEXT", "NVARCHAR", "TEXT", "VARCHAR", "NVARCHAR(MAX)", "VARCHAR(MAX)"):
            rtv_data_type = "String"
        elif data_type in ("DATE", "DATETIME", "TIME"):
            rtv_data_type = "Date"
        elif data_type in ("BIT"):
            rtv_data_type = "Boolean"
        elif data_type in ("BINARY", "VARBINARY", "VARBINARY(MAX)", "TIMASTAMP"):
            rtv_data_type = "Bytes"
...
def create_ftp_reader_json(table_schema_file_path, file_name):
    ...
    sftp_reader = {"name": "ftpreader", "parameter": {"protocol": "sftp", "host": ftp_host, "port": ftp_port,
                                                      "username": ftp_username, "password": ftp_password, "path": [
            "/upload/dingxiangClinic/yesterday/%s/%s" % (db_name, table_name)],
                                                      "column": column_list, "encoding": "UTF-8",
                                                      "fieldDelimiter": ","}}
    return sftp_reader


def create_sqlserver_reader_json():
    global column_type_list
    conn = sqlserver.connect(reader_db_info.get('host'),
                             reader_db_info.get('user'),
                             reader_db_info.get('password'),
                             reader_db_info.get('dbname')
                             )
    cursor = conn.cursor()
    db_name = get_reader_db_table_name()[0].upper()
    table_name = get_reader_db_table_name()[1].upper()
    ...
    where\
    d.name = '%s'\
    order By a.colorder" % table_name
    view_sql = "select \
        c.name as 字段名, \
        t.name as 数据类型, \
        null as comment \
        ...
        where objectproperty(id,'IsView') = 1 and id = object_id('NBC_YW.dbo.V_TAKE_IN_OUT_4_HIS')"
    cursor.execute(sql)
    count = 0
    for x in cursor.fetchall():
        s = (count, x[0], x[1], x[2].decode())
        count = count + 1
        column_type_list.append(s)
    if len(column_type_list) == 0:
        raise Exception("can not found the column info in %s, please check if the config of is right",
    ...
    column_list = list(map(lambda x: x[1], column_type_list))
    connection = [
        {"table": [reader_db_table_name], "jdbcUrl": ["jdbc:sqlserver://" + reader_db_info.get("listener")]}]
    sqlserver_reader = {"name": "sqlserverreader", "parameter": {"username": reader_db_info.get("user"),
                                                                 "password": reader_db_info.get("password"),
                                                                 "column": column_list, "splitPk": column_list[0],
                                                                 "connection": connection}}
    return sqlserver_reader


def parse_table_schema_cache(table_schema_file_path):
    """获取元数据信息"""
    global column_type_list
    # 修复windows下unicode报错
    if os.name=='nt':
        with open(table_schema_file_path, 'r',encoding='UTF-8') as file_to_read:
            ...
            count = 0
            for line in context:
                if re.search("create\s+table\s+", line.lower()) or re.search("constraint\s+", line.lower()) \
                        or re.search("\);", line.lower()):
                    continue
                if line.upper().find(' COMMENT ') > -1:
                    match_obj = re.match(r'(.*?)\s+(.*?)\s+.*(COMMENT\s+\'(.*?)\')', line.upper().strip())
...
def parse_table_schema_switch(table_schema_file_path):
    global ftp_db_type
    if reader_db_type == "cache":
        parse_table_schema_cache(table_schema_file_path)


def create_cache_reader_json(table_schema_file_path, file_name):
    global column_type_list
    column_type_list = []
    parse_table_schema_switch(table_schema_file_path)
    column_list = []
    for x in column_type_list:
        s = x[1]
        column_list.append(s)
    cache_host = reader_db_info.get("host")
    cache_port = reader_db_info.get("port")
    cache_username = reader_db_info.get("username")
    cache_password = reader_db_info.get("password")
    db_name = reader_db_info.get("dbname")
    table_name = file_name
    connection = [
        {"table": [table_name],
         "jdbcUrl": ["jdbc:Cache://%s:%s/%s" % (cache_host, cache_port, db_name)]}]
    cache_reader = {"name": "cachereader", "parameter": {"username": cache_username, "password": cache_password,
                                                           "column": column_list, "splitPk": column_list[0], "connection": connection}}
    return cache_reader
...
def create_reader_json(reader_type: str, table_schema_file_path, file_name):
    ...
    elif reader_type.lower() == "pg":
        return create_pg_reader_json()
    elif reader_type.lower() == "sqlserver":
        return create_sqlserver_reader_json()
    elif reader_type.lower() == "cache":
        return create_cache_reader_json(table_schema_file_path, file_name)
...
def create_sftp_writer_json():
    global column_type_list
    column_list = list(map(lambda x: x[1], column_type_list))
    db_name = reader_db_table_name.split('.')[0].lower()
    table_name = reader_db_table_name.split('.')[1].lower()
    path = "/".join(["/upload/jihtest", db_name + ".db", table_name])
    sftp_writer = {"name": "ftpwriter", "parameter": {
        "protocol": "sftp",
        "host": sftp_info.get("host"),
        "port": sftp_info.get("port"),
...
def create_writer_json(writer_type):
    ...
    elif writer_type == "sftp":
        return create_sftp_writer_json()
...
def create_datax_json(table_schema_file_path=None, file_name=None):
    ...
    reader_json = create_reader_json(reader_type=reader_db_type, table_schema_file_path=table_schema_file_path,
                                     file_name=file_name)
    ...
    with open(os.path.join(os.path.abspath(os.path.join(os.getcwd())), reader_db_table_name, work_dir_json,
                           reader_db_type + "2" + writer_db_type + "_" + reader_db_table_name + ".json"), "w+",
              encoding="utf-8") as f:
        f.write(datax_json)
...
def create_writer_hive_table_sql():
    ...
    with open(os.path.join(os.path.abspath(os.path.join(os.getcwd())), reader_db_table_name, work_dir_sql,
                           "create_table_%s_sql" % db_table_name), "w+", encoding="utf-8") as f:
        f.write("create external table IF NOT EXISTS %s (\r" % db_table_name)
...
if __name__ == '__main__':
    if reader_db_type == "ftp"or input_mode == "create_table_sql":
        do_batch()
    else:
        create_dir()
        create_datax_json()
