 ...
 **/
// todo 实现类似ProcessScheduleJob的逻辑
public class DependStateScheduleJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        FutureTask<String> future = threadRunner(DependStateCheckExecutor.class.getName());
        try {
            String resultReport = future.get(28L, TimeUnit.MINUTES);
            //send email
            emailManager.send(Arrays.asList(
                    "jack.yang@jiahui.com"
//                    ,"chacha.tang@jiahui.com"
     ...
     * @return
     */
    private FutureTask<String> threadRunner(String className) {
        FutureTask<String> futureTask = null;
        if (DependStateCheckExecutor.class.getName().equals(className)) {
            // new thread
            futureTask = new FutureTask<String>(new DependStateCheckExecutor());
